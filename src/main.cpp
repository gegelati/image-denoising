...
int main() {
	std::cout << "Start Denoising Toolchain  application." << std::endl;
	...
	Toolchain_denoise denoisingLE;

	std::cout << "Number of threads: " << std::thread::hardware_concurrency() << std::endl;

	// Instantiate and init the learning agent
	Learn::ParallelLearningAgent la(denoisingLE, set, params);
	la.init();
	...
	for (int i = 0; i < NB_GENERATIONS && !exitProgram; i++) {
		...
		if (printStats) {
            denoisingLE.denoise_data_test(la.getTPGGraph().getEnvironment(), la.getBestRoot().first);
			printStats = false;
		}
	}
    ...
    denoisingLE.denoise_data_test(la.getTPGGraph().getEnvironment(), la.getTPGGraph().getRootVertices().at(0));
