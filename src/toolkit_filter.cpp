...
void toolkit::mean_filter_3x3(vector<double>& in_img,vector<double>& out_img){
    ...
    toolkit::bin_to_Mat(in_img,img);         //Vector conversion from 1024 * 3 to an 32 * 32 image with 3 channels
    boxFilter(img,img,-1,cv::Size(3,3));      //Mean filtering 3x3
    toolkit::Mat_to_bin(img,out_img);          //Reconversion of the image into a vector of 1024 * 3
}
